...
const emailCryptoKey = CryptoJS.enc.Base64.parse(config.EMAIL_CRYPTO_KEY); // ! length=22 ! see Crypto.AES =>  If you use a passphrase, then use AES-256 ( 256-bit key).
...
const decryptEmail = (cryptedEmail) => {
	let decipher = CryptoJS.AES.decrypt(cryptedEmail, emailCryptoKey, {
		iv: emailIV,
	});
